...
<body>
	...
	<div>
				<table border="1" >
					...
					<tr>
					<td align = right height=10px>조회수 : 
					<%=bvo.getP_view()%></td>
					</tr>
					<tr>
						<td>
					...
					<td align = left>
					<% if(bvo.getNum()>1) {%>
					<a href="ViewService?pt_id=<%=bvo.getPt_id()%>&num=<%= bvo.getNum()-1%>" style="float: left;">이전</a>
					<% }else{ %>이전 <% } %>
					</td><td align = right>
					<% if(bvo.getNum()<maxnum) { %>
					<a href="ViewService?pt_id=<%=bvo.getPt_id()%>&num=<%= bvo.getNum()+1%>" style="float: right;">다음</a>
					<% }else{ %>다음 <% } %>
					</td>
					</td>
					</tr>
